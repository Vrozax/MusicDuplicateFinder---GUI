...
win = tk.Tk()
# global variables my_table, selected_folder_to_compare
...
def gui_start():
    ...
    def insert_values_to_table(save_data):
        ...
        for data in save_data:
           # ['H:/OneDrive/muzykaDoPrzeniesienia\\Curio - Ten Feet (Remix) .mp3', None, None, 320, 9901589]
            path=data[0]
            artist=data[1]
            title=data[2]
            bitrate=data[3]
            size=data[4]
            my_table.insert(parent='', index='end', iid=counter, text='', values=(counter, title, artist, bitrate,  size, path))
            counter+=1
    ...
    def start():
        ...
        if selected_folder_to_compare != None and selected_folder_to_compare != "":
            list_files = search_folder.search_files(str(selected_folder_to_compare))
            ...
            save_data=duplicate.compare_music(result)
